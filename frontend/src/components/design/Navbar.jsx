...
const Navbar = () => {
    ...
    const { notifications, markAllNotificationsAsRead, clearNotifications } = useOrderStore();
                    ...
                    <div className="hidden lg:flex items-center gap-2">
                        {authUser && (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent
                                    align="end"
                                    className="w-80 bg-background border border-border shadow-xl rounded-xl p-2 space-y-1.5"
                                >
                                    <div className="flex items-center justify-between px-3 py-1.5 border-b border-border/60 pb-2">
                                        <span className="text-xs font-bold text-foreground">Notifications</span>
                                        <div className="flex gap-2.5">
                                            <button
                                                onClick={markAllNotificationsAsRead}
                                                className="text-[10px] text-teal-600 hover:text-teal-700 font-semibold transition-colors"
                                            >
                                                Mark all read
                                            </button>
                                            <button
                                    ...
                                    <div className="max-h-[320px] overflow-y-auto pr-1 space-y-1 custom-scrollbar">
                                        {notifications.length === 0 ? (
                                            ...
                                            notifications.map(n => (
                                                <DropdownMenuItem
                                                    key={n.id}
                                                    className={`flex flex-col items-start gap-1 p-3 rounded-lg transition-colors cursor-pointer ${
                                                        !n.read 
                                                            ? "bg-teal-500/5 hover:bg-teal-500/10 border-l-2 border-teal-500 pl-2.5" 
