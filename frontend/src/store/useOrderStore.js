...
export const useOrderStore = create(
    devtools(
        persist(
                ...
                setupSocketListeners: () => {
                    ...
                    socket.on("newOrder", (newOrder) => {
                        ...
                        if (!recentOrders.find(o => o._id === newOrder._id)) {
                            ...
                            const newNotif = {
                                ...
                                title: "New Order",
                                description: `Order #${newOrder.orderId.split("-").pop().slice(-4)} placed for ${newOrder.type}`,
                                time: new Date(),
                                read: false
                            };
                            set(state => ({ notifications: [newNotif, ...state.notifications].slice(0, 50) })); // Keep last 50

                            // Trigger real-time toast and sound alert
                            toast.success(`New Order Received: #${newOrder.orderId.split("-").pop().slice(-4)}`, {
                                description: `${newOrder.type} • Rs ${newOrder.totalAmount.toFixed(2)}`,
                                duration: 5000,
                            });
                    ...
                    socket.on("orderStatusUpdate", (updatedOrder) => {
                        ...
                        const newNotif = {
                            id: Date.now() + Math.random().toString(),
                            title: "Order Status",
                            description: `Order #${updatedOrder.orderId.split("-").pop().slice(-4)} is now ${updatedOrder.status}`,
                            time: new Date(),
                            read: false
                        };
                        set(state => ({ notifications: [newNotif, ...state.notifications].slice(0, 50) })); // Keep last 50
                        ...
                        toast.info(`Order #${updatedOrder.orderId.split("-").pop().slice(-4)} Status Updated`, {
            ...
            {
                name: "order-storage",
                partialize: (state) => ({ cart: state.cart, notifications: state.notifications }), // only persist cart and notifications
            }
        )
    )
);
